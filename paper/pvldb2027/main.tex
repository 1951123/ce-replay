...
\centering
\small
\caption{Full-workload and exact incremental wall-clock results. Times are warm medians. Native planner is a black-box estimate-acquisition path, not isolated estimator time; native CE times only the clause-selectivity routine inside base-relation sizing.}
\label{tab:t4}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
\toprule
Workload / operation & Native planner & Native CE & Full Replay & Incremental result \\
\midrule
Census full workload & 90.867 ms & 10.241 ms & 1.884 ms & Planner/Replay 48.24$\times$; CE/Replay 5.44$\times$ \\
DMV full workload & 294.542 ms & 47.603 ms & 85.472 ms & Planner/Replay 3.45$\times$; Replay/CE 1.80$\times$ \\
Census sampled moves & --- & --- & Control & ADD 61.28$\times$; DROP 44.08$\times$; SWAP 31.49$\times$ \\
DMV sampled moves & --- & --- & Control & ADD 3.22$\times$; DROP 2.98$\times$; SWAP 1.92$\times$ \\
Fixed Census trajectory & --- & --- & 45.093 ms & 8.031 ms; 5.615$\times$ \\
\bottomrule
\end{tabularx}
\end{table*}
