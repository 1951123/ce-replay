...
\begin{abstract}
Selecting extended statistics for a supplied target workload is a physical-design problem with two distinct constraints: statistics can interact non-monotonically through the estimator, and deployed objects consume recurring maintenance capacity. Independent candidate scores are therefore insufficient because native applicability, winner selection, clause consumption, and mechanism composition make each candidate's effect contextual. We introduce CE-Replay, a workload-specialized, design-parametric representation that keeps the supported statistics-sensitive estimator semantics executable and exposes both an objective oracle and a semantic dependency oracle. We instantiate CE-Replay for PostgreSQL 16.14 conjunctive base-relation restrictions with MCV, functional-dependency, and constant \texttt{IN}/\texttt{= ANY} MCV semantics, and use it with deterministic constrained local search. CE-Replay matches fresh native estimates on all 468 Census and 1,965 DMV queries. Census supplies a mixed design under a calibrated maintenance-capacity proxy and a local-optimum certificate; across both workloads, exact incremental replay is faster than full replay in every tested move/scope cell, while full Replay versus isolated native CE remains workload-dependent. These results establish that executable native semantics can support statistics physical design within a validated fragment, while broader CE coverage, hypothetical-payload acquisition, payload-robust selection, and runtime objectives remain open.
\end{abstract}
\maketitle
...
\item \textbf{Semantics-guided optimization.} We use CE-Replay as a validated objective oracle and a statistics-semantic dependency oracle for deterministic maintenance-constrained ADD/DROP/SWAP search. Exact incremental move evaluation reproduces audited full replay and reduces controlled move-evaluation wall clock within the tested setting; workload-scale guarantees remain neighborhood-local rather than global.
\item \textbf{Cross-workload physical validation.} Census supplies a complete design and deployment case under a calibrated maintenance-capacity proxy, while dense IN-heavy DMV supplies semantic, non-monotonicity, physical-fidelity, and evaluator-performance validation. The evidence concerns CE loss and evaluator cost, not query-runtime improvement.
\end{enumerate}
...
Candidate definition, payload, and effective precedence are distinct. A definition identifies what PostgreSQL should collect; a payload contains sampled values, frequencies, dependency degrees, and their interpretation schema; and effective precedence captures the order observable by native statistics selection. PostgreSQL realizes that precedence through creation and OID-sorted catalog/list behavior, but CE-Replay consumes the induced order rather than raw OIDs. Re-running \texttt{ANALYZE} can change payload values or availability without changing the definition or precedence. Just-in-time, piggyback, and incremental collection address the complementary question of when and how to acquire or refresh statistics ~\cite{elhelw2007jits,zhu2004piggyback,pfeil2026redshift}.
...
for $N>0$. Estimate-side numerical protection floors $\widehat{N}$ at $10^{-300}$. Queries with zero ground truth remain in semantic-fidelity validation but are outside multiplicative q-error aggregation. Let $Q^+=\{q\in Q:N\_q>0\}$. The unweighted target-workload objective is
...
\textbf{Evaluated fixed-precedence problem.} The general formulation exposes selection and deployable effective precedence as parts of the physical state. The evaluated optimizer does not jointly optimize them: it fixes a recorded precedence, denoted by
...
Initially, all five predicates are unestimated, and both overlapping candidates \texttt{s1} and \texttt{s2} are applicable. GreedyCover prefers the statistic covering more currently unestimated semantic dimensions, so \texttt{s1} wins. Applying it jointly estimates and consumes the clauses on \texttt{A}, \texttt{B}, and \texttt{C}, leaving the explicit residual MCV state

\[
\{A,B,C,D,E\}\xrightarrow{s_1}\{D,E\}.
\]

The overlapping candidate \texttt{s2} was initially applicable but is no longer eligible after its clauses on \texttt{A} and \texttt{B} have been consumed. In contrast, the genuinely disjoint candidate \texttt{s3} remains reachable and can be selected in a later MCV round because \texttt{D} and \texttt{E} remain residual.

The FD stage runs after MCV processing and therefore observes this residual clause state. For the stated direction, \texttt{s1} has consumed the predicate on \texttt{C}. The FD stage no longer sees the original predicate configuration that supported the dependency's applicability, so the correction may be suppressed under the validated semantics.

The example exposes two interaction types. \textbf{Overlap competition:} \texttt{s1} can make the initially applicable \texttt{s2} ineligible. \textbf{Cross-mechanism state propagation:} MCV consumption changes the residual clauses observed by the downstream FD stage. Candidate utility is therefore not a fixed independent score; a selected design induces a sequence of native estimator state transitions.
...
\textbf{Figure~\ref{fig:f1}} separates hypothetical evaluation using frozen workload/payload context from physical deployment. Only the selected state enters the lower loop: deployment follows recorded precedence, fresh \texttt{ANALYZE} realizes new payloads, and replay is compared with native PostgreSQL on that same realization.
...
\item \textbf{Design-dependent state:} the selected definitions and effective precedence, represented by recorded PostgreSQL ranks within each native mechanism where order is observable.
...
\textbf{One move through CE-Replay.} For Section 4.1's query, start with \texttt{s2} and \texttt{s3}, then \texttt{ADD s1}. CE-Replay reuses specialized predicates, relation cardinality, ordinary selectivities, frozen candidate payloads and schemas, and recorded effective precedence. Loaded before search, these inputs remain fixed; payload acquisition stays outside the move loop.

The move re-executes design-dependent semantics. Before it, \texttt{s2} may cover \texttt{A,B}, while disjoint \texttt{s3} remains available for \texttt{D,E}. Under Section 4.1's GreedyCover condition, adding \texttt{s1} makes it win and consume \texttt{A,B,C}; \texttt{s2} becomes ineligible, \texttt{s3} remains reachable, and downstream FD reachability may change because \texttt{C} was consumed. Applicability, selection, consumption, reachability, and numerical contribution can change without changing payloads.
...
\subsection{Frozen and fresh payload realizations}

Optimization is conditional on the frozen payload repository. Deployment invokes \texttt{ANALYZE} and creates a fresh payload realization whose values or availability may differ. \textbf{Semantic replay error} is the replay/native discrepancy for one matched payload state and precedence. \textbf{Payload realization drift} is the difference between frozen and fresh payload outcomes. Figure~\ref{fig:f1} separates these quantities, and Section 7 reports them separately.
...
\begin{algorithm}[t]
\caption{Fixed-precedence local search using CE-Replay interfaces}
\label{alg:search}
\small
\begin{algorithmic}[1]
\State $Y \gets \emptyset$
\While{a feasible candidate has positive contextual benefit per maintenance unit}
  \State choose the deterministic best feasible candidate $s$
  \State $Y \gets Y \cup \{s\}$
  \State \Call{UpdateReplayState}{$Y$}
\EndWhile
\Loop
  \State $\mathit{best} \gets \varnothing$
  \For{each feasible ADD, DROP, or SWAP move $m$}
    \State $Q_{\mathrm{aff}} \gets \Call{DependencyOracle}{Y,m}$
    \State $\Delta \gets \Call{IncrementalObjective}{Y,m,Q_{\mathrm{aff}}}$
    \State $\mathit{best} \gets \Call{DeterministicBestImprovement}{\mathit{best},m,\Delta}$
  \EndFor
  \If{$\mathit{best}=\varnothing$ or is not a strict improvement}
    \State \Return $Y$
  \EndIf
  \State $Y \gets \Call{Commit}{Y,\mathit{best}}$
\EndLoop
\end{algorithmic}
\end{algorithm}
...
Ground-truth rows score replayed estimates using the q-error in Section 3. DMV queries \texttt{dmv.173} and \texttt{dmv.943} have zero truth: all 1,965 queries remain in semantic validation, while the objective aggregates only the 1,963 positive-truth queries. No DMV objective or design result that aggregates the zero-truth queries is used.

For the complete Census design study, recurring maintenance is instantiated with mechanism-weighted object counts calibrated from aggregate \texttt{ANALYZE} latency. The measured Census slopes are 1.874997 ms/MCV and 2.717261 ms/FD, yielding normalized costs 1 and 1.449. This first-order resource proxy is specific to the environment; it is neither a PostgreSQL constant nor a per-candidate latency predictor.
...
\centering
\small
\caption{Native fidelity in the supported fragment. Comparisons match query, design, effective precedence, and payload state; error is relative at the raw/native cardinality boundary.}
\label{tab:t2}
...
\textbf{Answer to RQ1.} Within the explicitly supported PostgreSQL 16.14 statistics-sensitive base-restriction fragment and matched payload state, CE-Replay matches native estimates to the reported floating-point tolerances. This result does not cover joins, full planner search, arbitrary predicates, other mechanisms, or other PostgreSQL versions.

\subsection{RQ2 --- Can replay drive resource-constrained statistics design?}

RQ2 first asks whether selection can remain necessary before capacity becomes binding. \textbf{Figure~\ref{fig:f2}} answers that question by separating semantic harm from maintenance scarcity.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth,height=1.86in,keepaspectratio]{figures/f2-cross-workload-nonmonotonicity.pdf}
\caption{\textbf{Cross-workload non-monotonicity.} Independently scaled panels compare empty and complete states. Census has 1,444 beneficial, seven neutral, and 1,560 harmful additions among 3,011 candidates, plus 317 improving removals; DMV has 30 beneficial and 41 harmful additions among 71 candidates, plus 20 improving removals. This is contextual evidence from two frozen payload realizations, not a universal property.}
...
Having established semantic need, \textbf{Table~\ref{tab:t3}} asks what the optimizer selects under the validated Census maintenance constraint.


\begin{table*}[t]
\centering
\small
\caption{Census calibrated maintenance-proxy design outcome. Local optimality is limited to the audited ADD/DROP/SWAP neighborhood under fixed payload and recorded effective precedence.}
\label{tab:t3}
...
\textbf{Answer to RQ2.} On Census, CE-Replay drives mixed statistics design under a calibrated maintenance-capacity proxy without a learned design-to-q-error response model. The state is an audited neighborhood local optimum under fixed precedence, not a global optimum.

\subsection{RQ3 --- Do semantic dependencies reduce move-evaluation work?}

RQ3 asks whether counterfactual-safe invalidation preserves exact move values, reduces replay work, and lowers evaluator cost. \textbf{Table~\ref{tab:t4}} separates black-box planner/oracle cost, isolated native CE, full Replay, and incremental Replay.
...
\centering
\small
\caption{Controlled evaluator-cost measurements. Times are warm medians. Panel A compares full-workload evaluation across the black-box native planner estimate-acquisition path, isolated native CE clause-selectivity boundary inside base-relation sizing, and complete in-memory Full Replay. Panel B compares exact Incremental Replay with Full Replay for move evaluation. Measurement boundaries differ, so ratios apply only within the shown comparisons; values above one mean Replay is faster, and values below one mean Replay is slower.}
\label{tab:t4}
\textbf{Panel A: Full-workload evaluator cost.}\par\smallskip
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth,height=2.00in,keepaspectratio]{figures/f4-mcv-fd-composition.pdf}
\caption{\textbf{Directed MCV-to-FD composition and FD consumption.} MCV clause consumption feeds FD residual state. In the earlier 105,061-byte Census experiment, independent mechanism selection chooses 97 FDs, 72 unused after composition, whereas the joint-semantic configuration chooses 54, all consumed. This is distinct from the final RQ2 calibrated-proxy state with seven FDs. In DMV, the all-statistics state consumes zero of 35 usable FDs; a fixed physical validation state contains 12 MCV and four FD objects and is used only for semantic fidelity, not RQ2.}
\Description{Final vector figure F4; its visual content is described by the caption and repository figure plan.}
\label{fig:f4}
\end{figure*}

The earlier byte-budget Census comparison in Figure~\ref{fig:f4} shows mixed-mechanism selection under fixed effective precedence avoiding many suppressed FDs; it is not the final seven-FD calibrated-proxy design reported for RQ2. DMV replicates all-statistics FD suppression under dense incidence. \textbf{Table~\ref{tab:t5}} then separates fresh matched-payload fidelity from frozen-to-fresh payload drift.


\begin{table*}[t]
\centering
\small
\caption{Physical deployment and fresh validation. Replay/native fidelity uses all queries; DMV q-error drift uses only its 1,963 positive-truth queries.}
\label{tab:t5}
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
\toprule
...
DMV diagnostic fixed state & 12 MCV + 4 FD; 12/12 MCV, 4/4 FD & Physical state validated & 1,965/1,965; 2.94e-14 & 46,034.243090 $\rightarrow$ 45,259.901254; -1.6821\% & Not a budget model \\
\bottomrule
\end{tabularx}
\end{table*}

For the final Census proxy-constrained design, all objects materialize, and all seven selected FDs are consumed. Table~\ref{tab:t5}'s 7.20\% cost error is $|0.829417164-0.773681909|/0.773681909$, comparing measured complete \texttt{ANALYZE} latency with the mechanism-level model prediction. Across 30 repeated \texttt{ANALYZE} payload realizations, 14,040/14,040 replay/native comparisons match while objective loss has 1.835\% coefficient of variation. This demonstrates matched-payload fidelity under measurable payload variation, not stability of design rankings. In DMV, all 16 objects materialize and all fresh queries match native. Its maintenance calibration did not satisfy the preregistered stability criterion, so the 12-MCV+4-FD state is retained only as a fixed-state semantic-fidelity test and no DMV budgeted-design result is reported.

\textbf{Answer to RQ4.} CE-Replay composes directed MCV and FD semantics and remains faithful after fresh physical deployment on both workloads. Fresh payload realization can change values and availability, so semantic replay error and payload realization drift must remain separate.
...
\subsection{Maintenance proxy and semantic utility}

The Census resource replaces bytes with mechanism-weighted counts fitted to aggregate \texttt{ANALYZE}. This calibrated first-order proxy is environment-specific, not a PostgreSQL constant or per-object latency predictor. DMV's calibration misses its preregistered stability criterion, so no DMV budgeted-design result is reported; the general additive formulation is not falsified.
...
\subsection{Frozen hypothetical state and fresh payload realization}

The optimizer is conditional on a frozen payload repository, while deployment generates a fresh payload realization. Replay/native fidelity within one matched payload state and frozen-to-fresh payload drift answer different questions. Repeated Census \texttt{ANALYZE} demonstrates payload and objective variability without semantic mismatch. It does not establish frequent design-ranking reversal or regret; that would require paired multi-design payload realizations.

Future objectives could optimize expected, risk-sensitive, or worst-case loss over payload realizations. Efficient acquisition of hypothetical candidate payloads is another open systems problem. Just-in-time, piggyback, and modern incremental statistics maintenance demonstrate complementary ways to reduce collection or refresh work ~\cite{elhelw2007jits,zhu2004piggyback,pfeil2026redshift}. CE-Replay does not implement those techniques: its offline acquisition boundary is distinct from the recurring maintenance cost of the selected deployment.
...
Fresh \texttt{ANALYZE} can change payloads or availability, and the optimizer is not payload-robust. DMV excludes two zero-truth queries from q-error but retains all 1,965 for semantic validation. Its fixed physical state supports fidelity and positive-truth drift, not a budgeted-design result.
