...
Selection also has an independent resource justification. Deployed extended statistics must be collected and refreshed by `ANALYZE`, and that recurring work depends on the mechanism and environment. A practical design must balance estimator utility against maintenance capacity: a statistic may be affordable yet harmful, or beneficial yet infeasible.
...
RQ4 asks whether the directed MCV-to-FD interaction survives complete design and physical deployment. **Figure F4** links that mechanism to observed FD reachability.
