...
$$
A=a \;\land\; B=b \;\land\; C=c \;\land\; D=d \;\land\; E=e,
$$

and a design containing

$$
s_1=\operatorname{MCV}(A,B,C),\qquad
s_2=\operatorname{MCV}(A,B),\qquad
s_3=\operatorname{MCV}(D,E),
$$

together with an FD whose useful direction is

$$
C\rightarrow D.
$$

Initially, all five predicates are unestimated, and both overlapping candidates `s1` and `s2` are applicable. GreedyCover prefers the statistic covering more currently unestimated semantic dimensions, so `s1` wins. Applying it jointly estimates and consumes the clauses on `A`, `B`, and `C`, leaving the explicit residual MCV state

$$
\{A,B,C,D,E\}\xrightarrow{s_1}\{D,E\}.
$$

The overlapping candidate `s2` was initially applicable but is no longer eligible after its clauses on `A` and `B` have been consumed. In contrast, the genuinely disjoint candidate `s3` remains reachable and can be selected in a later MCV round because `D` and `E` remain residual.

The FD stage runs after MCV processing and therefore observes this residual clause state. For the stated direction, `s1` has consumed the predicate on `C`. The FD stage no longer sees the original predicate configuration that supported the dependency's applicability, so the correction may be suppressed under the validated semantics.

The example exposes two interaction types. **Overlap competition:** `s1` can make the initially applicable `s2` ineligible. **Cross-mechanism state propagation:** MCV consumption changes the residual clauses observed by the downstream FD stage. Candidate utility is therefore not a fixed independent score; a selected design induces a sequence of native estimator state transitions.
...
CE-Replay is a workload-specialized executable representation of the supported statistics-sensitive estimator fragment, not the search algorithm, complete DBMS, or native estimator. It specializes workload-fixed context while leaving design-dependent applicability, winner selection, clause consumption, residual state, numerical updates, FD reachability, and relevant precedence executable across hypothetical designs. Those transitions drive both objective evaluation and counterfactual-safe dependency/invalidation; freezing one observed calculation would lose that structure.

**Figure F1** separates hypothetical evaluation using frozen workload/payload context from physical deployment. Only the selected state enters the lower loop: deployment follows recorded precedence, fresh `ANALYZE` realizes new payloads, and replay is compared with native PostgreSQL on that same realization.
...
**One move through CE-Replay.** For Section 4.1's query, start with `s2` and `s3`, then `ADD s1`. CE-Replay reuses specialized predicates, relation cardinality, ordinary selectivities, frozen candidate payloads and schemas, and recorded effective precedence. Loaded before search, these inputs remain fixed; payload acquisition stays outside the move loop.

The move re-executes design-dependent semantics. Before it, `s2` may cover `A,B`, while disjoint `s3` remains available for `D,E`. Under Section 4.1's GreedyCover condition, adding `s1` makes it win and consume `A,B,C`; `s2` becomes ineligible, `s3` remains reachable, and downstream FD reachability may change because `C` was consumed. Applicability, selection, consumption, reachability, and numerical contribution can change without changing payloads.

Repeated native evaluation invokes the DBMS planner/estimator path per design. Full Replay reuses frozen context and executes supported semantics for the complete workload; its cost relative to isolated native CE remains workload-dependent. Incremental Replay uses structural/counterfactual dependency scope---not only the realized trace---to revisit affected queries and avoid unchanged control, numerical, and objective contributions. CE-Replay separates design-invariant context from design-dependent transitions, which its semantic dependency oracle makes reusable across neighboring designs.
...
> **Figure F4: Directed MCV-to-FD composition and FD consumption.** MCV clause consumption feeds FD residual state. In the earlier 105,061-byte Census experiment, independent mechanism selection chooses 97 FDs, 72 unused after composition, whereas the joint-semantic configuration chooses 54, all consumed. This is distinct from the final RQ2 calibrated-proxy state with seven FDs. In DMV, the all-statistics state consumes zero of 35 usable FDs; a fixed physical validation state contains 12 MCV and four FD objects and is used only for semantic fidelity, not RQ2. *(Production note: render the frozen F4 specification.)*

The earlier byte-budget Census comparison in Figure F4 shows mixed-mechanism selection under fixed effective precedence avoiding many suppressed FDs; it is not the final seven-FD calibrated-proxy design reported for RQ2. DMV replicates all-statistics FD suppression under dense incidence. **Table T5** then separates fresh matched-payload fidelity from frozen-to-fresh payload drift.
