...
Combinatorial design requires evaluating many hypothetical statistics states. Independent candidate rankings cannot represent stateful interactions, while repeatedly materializing every state and invoking the native system is an awkward inner loop and does not directly expose dependencies for incremental evaluation. Existing what-if and response-reuse approaches motivate careful comparison rather than a claim that hypothetical physical design is new [CITATION NEEDED: optimizer what-if interfaces for physical design] [CITATION NEEDED: INUM-style optimizer response specialization]. The specific challenge here is to retain the estimator behavior that generates statistics interactions.
...
3. **Semantics-guided optimization.** We use CE-Replay as an exact-within-scope objective oracle and a semantic dependency oracle for deterministic maintenance-constrained ADD/DROP/SWAP search and exact incremental move evaluation. Workload-scale guarantees are neighborhood-local rather than global.
...
A query optimizer estimates predicate selectivity and intermediate cardinality to compare candidate plans. Per-column statistics often lead to an independence approximation for multiple predicates; correlations can make that approximation inaccurate. Multivariate statistics provide additional information for selected attribute groups [CITATION NEEDED: classical selectivity estimation and attribute independence] [CITATION NEEDED: PostgreSQL extended-statistics documentation and design].
...
FD processing is not another GreedyCover instance. PostgreSQL identifies dependency entries compatible with residual equality-eligible predicates, aggregates their degrees, and adjusts selectivity while accounting for implied attributes. Applicability depends on predicate form, semantic dimensions, payload availability, and the state entering the FD stage.
...
Future objectives could optimize expected, risk-sensitive, or worst-case loss over realizations. Efficient acquisition of hypothetical candidate payloads is another open systems problem. It is a one-time offline cost distinct from recurring deployed maintenance, not a capability solved by CE-Replay.
...
This section is a citation-aware skeleton pending a dedicated literature pass. It makes no external priority claim.

### 10.1 Automated physical design

Automated physical-design research studies selection of persistent structures under workload and resource constraints [CITATION NEEDED: surveys and foundational index/materialized-view advisors]. Relevant comparisons include candidate generation, workload objectives, resource models, and use of native what-if interfaces [CITATION NEEDED: DBMS what-if physical-design interfaces]. CE-Replay focuses on statistics objects that change estimator information rather than primarily adding access alternatives.

### 10.2 Fast hypothetical optimizer evaluation

Prior approaches reuse or specialize optimizer response to reduce repeated what-if evaluation [CITATION NEEDED: INUM and related plan-cost response caching] [CITATION NEEDED: modern hypothetical-configuration evaluation]. The later literature pass must compare what is cached, which state remains executable, and what guarantees hold. CE-Replay specializes a supported CE response and exposes semantic dependencies; it does not claim that prior systems merely enumerate blindly.

### 10.3 Cardinality estimation

Classical CE combines catalog statistics and modeling assumptions [CITATION NEEDED: classical cardinality/selectivity estimation]. Multivariate and extended statistics address dependence missed by single-column summaries [CITATION NEEDED: multivariate database statistics]. Learned CE methods estimate cardinalities or model data/query distributions [CITATION NEEDED: learned cardinality-estimation surveys and representative systems]. CE-Replay instead retains the native estimator and evaluates how physical statistics states change its supported behavior.

### 10.4 Extended-statistics selection and configuration

Work on automatically recommending or configuring multivariate statistics is the closest problem category [CITATION NEEDED: workload-driven multivariate statistics selection] [CITATION NEEDED: automatic extended-statistics creation in commercial and open-source DBMSs]. The literature pass must compare semantic interaction models, objective definitions, hypothetical payload assumptions, and maintenance constraints without asserting priority.

### 10.5 Positioning of CE-Replay

CE-Replay occupies the boundary between CE implementation and physical design. It is not a replacement estimator, a full planner emulator, or the local-search algorithm. Its distinguishing internal abstraction is a workload-specialized executable representation that supplies both an objective oracle and a dependency oracle for supported statistics semantics. External novelty and priority relative to existing systems remain to be established by the literature pass.
